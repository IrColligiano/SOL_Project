...
int write_on_socket(char* path, long res, size_t len) {
    ...
    if ((w = write_n(fd_skt, &res, sizeof( long ))) < 0) {
        fprintf(stderr,"ERRORE: write_n result\n");
        //printf("prova matteo00000 %ld\n", res);
        return -1;
    }
    //printf("prova matteo1 %ld\n", res);
    if (w == 0) {
...
int write_on_socket_term_request(){
    ...
    if ((w = write_n(fd_skt, &print_val, sizeof(long))) < 0){
...
void * thread_work(void * arg){
    ...
    if(th_pool->n_th_on_work <= 0){
        ...
        if(( w = write_on_socket_term_request()) <= 0 ){
            fprintf(stderr,"ERRORE: write on socket term request\n");
        }
